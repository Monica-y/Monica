...
class KAnonymity():
    ...
    def anonymize(self, qi_names=['age', 'education'], k=5):
        """
        ...
        # assert condition的意义是令程序测试condition，并在condition测试结果为false的时候抛出异常。
        ...
        for qiname in qi_names:
            # set 数组是一组key值，不包含重复元素。不支持索引和切片操作。
            # 为每一个关键字新建一个set
            domains[qiname] = set()
            gen_levels[qiname] = 0
        
        for idx, record in enumerate(self.records):
            # 类似于C++的浅拷贝和深拷贝，record[:]是深拷贝
            qi_sequence = self._get_qi_values(record[:], qi_names, generalize_tree)
            #　如果qi_frequency中没有这个set，就说明没有为qi_sequence这条记录定义set
        ...
        while True:
            ...
            if negcount > k:
                ...
                #'F:\\大数据安全\\第三次实验\\实验代码\\data'
                with open('F:\\大数据安全\\第三次实验\\实验代码\\data\\adult_%d_kanonymity.data' %k, 'w') as wf:
