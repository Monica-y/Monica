...
def socket_client():
    ...
    while True:
        mode = input("please input your file type (img or else)")
        ...
        if not mode == 'img':
            ...
            if os.path.isfile(filepath):
                ...
                while True:
                    ...
                    if not data:
                        ...
                        fp.close()
                        break
                    data = myAES(key,bytes.decode(data,'utf-8','ingore'),ENCRYPT)      
                    clientSocket.send(data.encode())
                    print(clientSocket.recv(BUFSIZ)) # edit
            # 如果要实现同一文件多次加密结果不同，取消下面注释
            # key = Clientmd5.md5Encode()
        else:# edit
            filepath = input("please input file path:")
            if os.path.isfile(filepath):
            ...
            os.stat(filepath).st_size)
                ...
                while True:
                    ...
                    aes = AES.new(key_16,AES.MODE_ECB) #创建一个aes对象
                    en_data = aes.encrypt(data)
                    clientSocket.send(en_data)
                    print(clientSocket.recv(BUFSIZ))
...
# https://blog.csdn.net/weixin_34910922/article/details/117536110